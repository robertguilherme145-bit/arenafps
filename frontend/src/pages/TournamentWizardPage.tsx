...
const schema = z.object({
  ...
  premiacao: z.string().trim().min(1, "Informe a premiação."),
...
}).refine((data) => data.payment_mode === "free" || data.valor > 0, { path:["valor"], message:"Informe o valor da inscrição." });

type WizardFormInput = z.input<typeof schema>;
type WizardForm = z.output<typeof schema>;

const steps = [
  { title: "Informações", description: "Identidade e jogo" },
  { title: "Datas", description: "Periodo da competição" },
  { title: "Inscrições", description: "Vagas, lineup e pagamento" },
  { title: "Formato", description: "Estrutura e series" },
  { title: "Regras", description: "Operação da partida" },
  { title: "Map Pool", description: "Mapas e ordem de veto" },
  { title: "Premiação", description: "Premios e apresentacao" },
  { title: "Resumo", description: "Revisão completa" },
  { title: "Publicação", description: "Criar na plataforma" }
];
...
export function TournamentWizardPage() {
  ...
  async function nextStep() {
    ...
    if (!valid) {
      toast.warning("Revise está etapa", "Corrija os campos indicados antes de continuar.");
      return;
    }
    if (step === 5 && values.pick_ban_enabled) {
      ...
      if (completedOrder.length > selectedMapIds.length) {
        toast.warning("Ordem de veto invalida", "Não restam mapas suficientes para completar a serie automaticamente.");
        return;
      }
  ...
  function handleInvalidPublish(validationErrors: FieldErrors<WizardFormInput>) {
    ...
    toast.warning("Não foi possivel publicar", message);
  }
  ...
  const previewItems = useMemo(() => [
    ["Jogo", selectedGame?.nome ?? "Não selecionado"],
    ...
    ["Inscrição", values.payment_mode === "free" ? "Gratuita" : `R$ ${Number(values.valor).toFixed(2).replace(".",",")} por jogador`],
  ...
  return (
    <section className="px-4 pb-12 lg:px-8">
      <PageHeader eyebrow="Competicoes" title="Criar torneio" description="Configure cada regra antes da publicação. Nenhum formato ou mapa e fixo no sistema." />
      <div className="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)_320px]">
        ...
        <Card className="self-start xl:sticky xl:top-20">
          ...
          <CardContent className="space-y-4">
            <div className="border border-cyan-400/25 bg-cyan-400/10 p-4"><Trophy className="h-7 w-7 text-cyan-200" /><h3 className="mt-4 font-display text-xl font-bold">{values.nome || "Novo torneio"}</h3><p className="mt-2 text-sm text-arena-muted">{values.descricao || "A descrição aparecera aqui."}</p></div>
            ...
            <Button className="w-full" icon={<RotateCcw className="h-4 w-4" />} variant="ghost" onClick={clearDraft}>Limpar configuração</Button>
          </CardContent>
        </Card>
      </div>
    </section>
