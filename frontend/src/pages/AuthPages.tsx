...
export function LoginPage() {
  ...
  useEffect(() => { if (oauthError && !reportedOAuthError.current) { reportedOAuthError.current = true; error("Não foi possivel entrar", oauthError); } }, [oauthError]);
  ...
  async function submit() {
    ...
    } catch (reason) { error("Não foi possivel entrar", message(reason)); }
  ...
  return <AuthShell eyebrow="Conta Arena Camp" title="Bem-vindo de volta" description="Acesse sua carreira, equipe e competicoes com uma unica conta.">
    <div className="space-y-4">
      ...
      <p className="pt-2 text-center text-sm text-arena-muted">Ainda não possui conta? <Link className="font-semibold text-cyan-200 hover:underline" to="/criar-conta">Criar conta</Link></p>
    </div>
  </AuthShell>;
}
...
export function RegisterPage() {
  ...
  async function finish() {
    ...
    catch (reason) { error("Não foi possivel criar a conta", message(reason)); }
    finally { setBusy(false); }
  }
  if (result) return <AuthShell eyebrow="Cadastro concluido" title={result.email_sent ? "Confirme seu email" : "Conta criada"} description={result.email_sent ? `Enviamos as instrucoes para ${result.email}.` : "Não foi possivel enviar o email. Solicite um novo link na tela de acesso."}><div className="space-y-4 text-center"><MailCheck className="mx-auto h-12 w-12 text-cyan-200" /><p className="text-sm leading-6 text-arena-muted">A confirmação protege sua conta e libera inscrições, presencas e demais ações competitivas.</p><Link to="/entrar"><Button className="w-full" variant="secondary">Ir para o login</Button></Link></div></AuthShell>;
  ...
  return <AuthShell eyebrow={`Etapa ${step} de 4`} title="Criar sua conta" description="Uma identidade para todos os jogos, equipes e papeis da Arena Camp.">
    <div className="mb-6 grid grid-cols-4 gap-2">{["Conta", "Jogos", "Vinculos", "Revisão"].map((label, index) => <div key={label}><div className={`h-1 ${step >= index + 1 ? "bg-cyan-400" : "bg-white/10"}`} /><p className="mt-2 hidden text-xs text-arena-muted sm:block">{label}</p></div>)}</div>
...
export function OAuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    void loginWithOAuthCode(code)
      ...
      .catch((reason) => { error("Não foi possivel concluir", message(reason)); navigate("/entrar", { replace:true }); });
  }, []);
...
export function OnboardingPage() {
  ...
  const options = [{ role:"jogador" as const, title:"Sou jogador", description:"Construa sua carreira, encontre equipes e acompanhe partidas.", icon:<UserRound className="h-6 w-6" /> }, { role:"lider" as const, title:"Sou líder de equipe", description:"Crie e gerencie elenco, lineups e inscrições.", icon:<Users className="h-6 w-6" /> }];
  async function choose(role: typeof options[number]["role"]) { setBusy(role); try { await completeAccountOnboarding(role); await refreshSession(); navigate(roleHref(role)); } catch (reason) { error("Não foi possivel concluir", message(reason)); } finally { setBusy(null); } }
  if (user?.onboarding_completed) return <Navigate replace to={roleHref(user.active_role)} />;
  if (!user) return <AuthShell eyebrow="Primeiro acesso" title="Preparando sua conta" description="Carregando seus contextos e permissões."><div className="flex justify-center py-8"><LoaderCircle className="h-9 w-9 animate-spin text-cyan-200" /></div></AuthShell>;
  if (!gamesLoaded) return <AuthShell eyebrow="Primeiro acesso" title="Preparando sua conta" description="Carregando o catálogo competitivo."><div className="flex justify-center py-8"><LoaderCircle className="h-9 w-9 animate-spin text-cyan-200" /></div></AuthShell>;
  if (user.needs_email) {
