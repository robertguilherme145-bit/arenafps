...
export function CaptainDashboardPage() {
  ...
  return <section className="px-4 pb-12 lg:px-8">
    <PageHeader eyebrow="Capitão da equipe" title={workspace.captain.nick || "Capitão"} description={`${workspace.captain.team_name} · ${workspace.captain.game_name} · Operacao competitiva`} action={<Link to={`/equipe/${workspace.captain.team_slug}`}><Button icon={<ExternalLink className="h-4 w-4" />} variant="secondary">Equipe pública</Button></Link>} />
...
function CaptainContext({ data, busy, onReload }: { data: CaptainWorkspace; busy: string | null; onReload: () => void }) {
  ...
  return <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
    <StatCard label="Próxima partida" value={next ? formatShortDate(next.scheduled_at) : "Sem agenda"} helper={next?.opponent || "Aguardando chaveamento"} icon={<Swords className="h-5 w-5" />} />
...
function DashboardModule({ data, openModule }: { data: CaptainWorkspace; openModule: (module: CaptainModule) => void }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
    <div className="space-y-5"><Card><CardHeader><h2 className="font-display text-xl font-semibold">Central do dia</h2></CardHeader><CardContent className="grid gap-3 sm:grid-cols-3"><QuickAction label="Presenças" value={data.matches.filter((match) => !["finalizada", "cancelada"].includes(match.status) && !match.attendance_status).length} action="Abrir partidas" onClick={() => openModule("matches")} /><QuickAction label="Pick & Ban" value={data.matches.filter((match) => !["finalizada", "cancelada"].includes(match.status) && match.veto_status === "liberado").length} action="Entrar na sala" onClick={() => openModule("veto")} /><QuickAction label="Notificações" value={data.notifications.filter((item) => !item.lida).length} action="Ver avisos" onClick={() => openModule("communication")} /></CardContent></Card><Card><CardHeader><h2 className="font-display text-xl font-semibold">Próximas partidas</h2></CardHeader><CardContent className="space-y-3">{upcoming.map((match) => <MatchSummary key={match.id} match={match} />)}{!upcoming.length ? <InlineEmpty text="Nenhuma partida agendada para a próxima rodada." /> : null}</CardContent></Card><Card><CardHeader><h2 className="font-display text-xl font-semibold">Últimos resultados</h2></CardHeader><CardContent className="space-y-3">{results.map((match) => <MatchSummary key={match.id} match={match} />)}{!results.length ? <InlineEmpty text="Os resultados oficiais aparecerão aqui." /> : null}</CardContent></Card></div>
...
function MatchesModule({ data, busy, run }: ModuleProps) {
  ...
  async function attendance(status: "confirmado" | "ausente" | "talvez") {
    ...
    const ok = await run(`attendance-${selectedId}`, () => updateCaptainMatchAttendance(selectedId, status), "Presença atualizada");
    if (ok) await reload();
  }
  ...
  return <div className="grid gap-5 xl:grid-cols-[350px_1fr]">
    ...
    {loading ? <Card><CardContent><Skeleton className="h-96" /></CardContent></Card> : room ? <div className="space-y-5"><Card><CardContent><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><StatusBadge value={room.match.status} /><Badge tone="info">{room.match.best_of.toUpperCase()}</Badge></div><h2 className="mt-3 font-display text-2xl font-bold">{room.match.team_a} vs {room.match.team_b}</h2><p className="mt-2 text-sm text-arena-muted">{room.match.tournament_name} · Rodada {room.match.round} · {formatDate(room.match.scheduled_at)}</p></div><Button icon={<Trophy className="h-4 w-4" />} variant="secondary" onClick={() => void openTournamentCenter(room.match.tournament_id)}>Central do torneio</Button></div></CardContent></Card>
      {!['finalizada', 'cancelada'].includes(room.match.status) ? <Card><CardHeader><h3 className="font-display text-lg font-semibold">Disponibilidade para a partida</h3></CardHeader><CardContent><div className="flex flex-wrap items-center justify-between gap-4"><p className="text-sm text-arena-muted">Sua resposta fica visível para o líder e pode ser alterada enquanto a partida estiver aberta.</p><AttendanceResponse busy={busy === `attendance-${selectedId}`} value={room.attendance?.status} onChange={(status) => void attendance(status)} /></div></CardContent></Card> : null}
...
function LineupModule({ data, busy, run }: ModuleProps) {
  ...
  return <div className="space-y-5"><div className="grid gap-5 xl:grid-cols-2">{data.lineups.map((lineup) => <Card key={lineup.entry_id}><CardHeader><div className="flex items-center justify-between gap-3"><div><h2 className="font-display text-xl font-semibold">{lineup.lineup_name}</h2><p className="mt-1 text-sm text-arena-muted">{lineup.tournament_name}</p></div><StatusBadge value={lineup.entry_status} /></div></CardHeader><CardContent className="space-y-3">{lineup.players.map((player) => <div className="flex items-center justify-between border border-arena-line p-3" key={player.player_id}><div><p className="font-semibold">{player.nick}</p><p className="text-xs text-arena-muted">ID #{player.player_id} · {player.game_uid || "ID do jogo pendente"}</p></div><div className="flex gap-2"><Badge tone={player.titular ? "success" : "info"}>{player.titular ? "Titular" : "Reserva"}</Badge><StatusBadge value={player.confirmado ? "confirmado" : "pendente"} /></div></div>)}</CardContent></Card>)}{!data.lineups.length ? <EmptyState title="Nenhuma lineup oficial" description="O líder ainda não enviou uma lineup para torneios." /> : null}</div>
    ...
    <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-display text-xl font-semibold">Elenco da equipe</h2><p className="mt-1 text-sm text-arena-muted">Leitura operacional. Cargos e lineups sao definidos pelo líder.</p></div>{!data.permissions.remove_players ? <Badge tone="neutral">Gestao exclusiva do líder</Badge> : null}</div></CardHeader><DataTable data={data.members} columns={[{ header: "Jogador", cell: (member) => <div><p className="font-semibold">{member.nick || member.nickname || member.nome}</p><p className="text-xs text-arena-muted">#{member.player_id ?? "-"} · {member.game_uid || "ID pendente"}</p></div> }, { header: "Cargo", cell: (member) => roleLabel(member.cargo) }, { header: "Escalacao", cell: (member) => <Badge tone={member.lineup_status === "titular" ? "success" : "info"}>{member.lineup_status}</Badge> }, { header: "Status", cell: (member) => <StatusBadge value={member.status} /> }, { header: "Ação", cell: (member) => data.permissions.remove_players && member.cargo === "player" ? <Button className="h-8 text-xs" variant="danger" onClick={() => confirmAction("Remover este jogador da equipe?", () => run(`remove-${member.id}`, () => removeCaptainMember(member.id), "Jogador removido"))}>Remover</Button> : <span className="text-xs text-arena-muted">Somente leitura</span> }]} /></Card>
  </div>;
}

function CalendarModule({ data, busy, run }: ModuleProps) {
  async function matchAttendance(matchId: number, status: "confirmado" | "ausente") { await run(`match-att-${matchId}`, () => updateCaptainMatchAttendance(matchId, status), "Presença atualizada"); }
  async function eventAttendance(eventId: number, status: "confirmado" | "ausente" | "talvez") { await run(`event-att-${eventId}`, () => updateCaptainEventAttendance(eventId, status), "Presença atualizada"); }
  return <div className="grid gap-5 xl:grid-cols-2"><Card><CardHeader><h2 className="font-display text-xl font-semibold">Partidas oficiais</h2></CardHeader><CardContent className="space-y-3">{data.matches.map((match) => <div className="border border-arena-line p-4" key={match.id}><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">vs {match.opponent}</p><p className="mt-1 text-sm text-arena-muted">{match.tournament_name} · {formatDate(match.scheduled_at)}</p></div><StatusBadge value={match.status} /></div>{!["finalizada", "cancelada"].includes(match.status) ? <div className="mt-3 border-t border-arena-line pt-3"><AttendanceResponse allowMaybe={false} busy={busy === `match-att-${match.id}`} compact value={match.attendance_status} onChange={(status) => void matchAttendance(match.id, status as "confirmado" | "ausente")} /></div> : null}</div>)}{!data.matches.length ? <InlineEmpty text="Nenhuma partida no calendário." /> : null}</CardContent></Card><Card><CardHeader><h2 className="font-display text-xl font-semibold">Treinos e eventos</h2></CardHeader><CardContent className="space-y-3">{data.events.map((event) => <div className="border border-arena-line p-4" key={event.id}><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{event.title}</p><p className="mt-1 text-sm text-arena-muted">{formatDate(event.starts_at)}{event.location ? ` · ${event.location}` : ""}</p></div><Badge tone="info">{event.type}</Badge></div><div className="mt-3 border-t border-arena-line pt-3"><AttendanceResponse busy={busy === `event-att-${event.id}`} compact value={event.attendance_status} onChange={(status) => void eventAttendance(event.id, status)} /></div></div>)}{!data.events.length ? <InlineEmpty text="Nenhum treino ou evento cadastrado." /> : null}</CardContent></Card></div>;
}
...
function LeaveTeamCard({ data, busy, run }: ModuleProps) {
  ...
  async function leaveTeam() {
    const ok = await run("leave-team", () => leaveCaptainTeam(confirmation), "Você saiu da equipe", false);
    if (ok) window.location.assign("/jogador?status=team-left");
  }

  return <>
    <Card className="border-red-500/35">
      <CardHeader><h2 className="font-display text-xl font-semibold">Vinculo com a equipe</h2><p className="mt-1 text-sm text-arena-muted">A saida encerra suas permissões de capitão, mas preserva todo o histórico competitivo.</p></CardHeader>
    ...
    <Modal open={open} title="Sair da equipe" description="Esta ação remove imediatamente seu vinculo de capitão." onClose={() => { setOpen(false); setConfirmation(""); }}>
      <div className="space-y-4">
        {(pendingMatches || activeTournaments) ? <div className="border border-amber-500/35 bg-amber-500/10 p-4 text-sm text-amber-100"><p className="font-semibold">Atencao ao calendário competitivo</p><p className="mt-1">Existem {pendingMatches} partidas pendentes e {activeTournaments} torneios ativos vinculados a equipe. A organizacao e o líder continuarao com o histórico da lineup.</p></div> : null}
        <div className="space-y-2 text-sm text-arena-muted"><p>Ao confirmar:</p><p>• você perde acesso ao workspace do capitão;</p><p>• seu cadastro antigo fica inativo na equipe;</p><p>• partidas, resultados e estatísticas permanecem registrados;</p><p>• sua conta volta para o perfil Jogador.</p></div>
...
function roleLabel(value: string) { return ({ leader: "Líder", captain: "Capitão", manager: "Manager", player: "Jogador", lider: "Líder", capitao: "Capitão", jogador: "Jogador", admin: "Admin" } as Record<string, string>)[value] ?? value; }
