...
function PublicPlayerProfile({ slug }: { slug: string }) {
  ...
  async function share() {
    ...
    else { await navigator.clipboard.writeText(payload.url); toast.success("Link copiado", "O perfil esta pronto para compartilhar."); }
  }
  ...
  if (notFound || !data) return <section className="px-4 py-12 lg:px-8"><EmptyState title="Perfil indisponivel" description="O jogador nao existe ou escolheu manter a carreira privada." /></section>;
...
function PublicTeamProfile({ slug }: { slug:string }) {
  ...
  if(!data)return <section className="px-4 py-12 lg:px-8"><EmptyState title="Equipe indisponivel" description="Esta equipe nao existe ou nao esta ativa."/></section>;
