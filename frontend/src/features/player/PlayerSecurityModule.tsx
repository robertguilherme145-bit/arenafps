...
export function PlayerSecurityModule({ data, busy, run }: PlayerModuleProps) {
  ...
  return <div className="grid gap-5 xl:grid-cols-2">
    <Card><CardHeader><div className="flex items-center gap-3"><Save className="h-5 w-5 text-cyan-200" /><div><h2 className="font-display text-xl font-semibold">Preferências</h2><p className="text-sm text-arena-muted">Tema, idioma, privacidade e notificações.</p></div></div></CardHeader><CardContent className="space-y-4"><Field label="Tema"><Select value={preferences.theme} onChange={(event) => setPreferences({ ...preferences, theme: event.target.value as typeof preferences.theme })}><option value="dark">Escuro</option><option value="light">Claro</option><option value="system">Usar configuração do sistema</option></Select></Field><Field label="Idioma"><Select value={preferences.language} onChange={(event) => setPreferences({ ...preferences, language: event.target.value })}><option value="pt-BR">Português (Brasil)</option><option value="en-US">English</option><option value="es-ES">Español</option></Select></Field><Toggle label="Perfil público" description="Permitir que outros usuários consultem sua carreira." checked={preferences.profile_public} onChange={(value) => setPreferences({ ...preferences, profile_public: value })} /><Toggle label="Notificações por e-mail" description="Receber avisos importantes no e-mail cadastrado." checked={preferences.email_notifications} onChange={(value) => setPreferences({ ...preferences, email_notifications: value })} /><Toggle label="Notificações no Discord" description="Receber integracoes quando houver um Discord vinculado." checked={preferences.discord_notifications} onChange={(value) => setPreferences({ ...preferences, discord_notifications: value })} /><Button loading={busy === "preferences"} icon={<Save className="h-4 w-4" />} onClick={() => void run("preferences", () => updatePlayerWorkspaceSettings(preferences), "Preferências atualizadas")}>Salvar preferências</Button></CardContent></Card>
